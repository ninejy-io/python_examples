...
class GoodsImageSerialier(serializers.ModelSerializer):
    class Meta:
        model = GoodsImage
        fields = ("image",)
...
class GoodsSerializer(serializers.ModelSerializer):
    ...
    images = GoodsImageSerialier(many=True)
    class Meta:
        model = Goods
        fields = '__all__'
...
class IndexCategorySerializer(serializers.ModelSerializer):
    ...
    def get_ad_goods(self, obj):
        goods_json = {}
        _ad_goods = IndexAd.objects.filter(category_id=obj.id)
        if _ad_goods:
            goods_instance = _ad_goods[0].goods
