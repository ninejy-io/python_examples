...
class StatCounter(object):
    ...
    def get_stats(self):
        return {
            "Overall_Stats": self._general_stats(),
            "Top_Prefixes": self._top_n(self.prefixes),
            "Top_Keys": self._top_n(self.keys),
            "Top_Commands": self._top_n(self.commands),
            "Command_Time(microsecs)": self._time_stats(self.times),
            "Heaviest_Commands(microsecs)": self._heaviest_commands(self.times),
            "Slowest_Calls": self._slowest_commands(self.times)
        }

    def handle_data(self):
        data = self.get_stats()
        return {
            "TopKeys": {'top_keys': [i[0] for i in data['Top_Keys']], 'data': [i[1] for i in data['Top_Keys']]},
            "TopCommands": {'top_commands': [i[0] for i in data['Top_Commands']], 'data': [i[1] for i in data['Top_Commands']]},
            "CommandTime": {'command_time': [i[0] for i in data['Command_Time(microsecs)']], 'data': [i[1] for i in data['Command_Time(microsecs)']]},
            "HeaviestCommands": {'heaviest_commands': [i[0] for i in data['Heaviest_Commands(microsecs)']], 'data': [i[1] for i in data['Heaviest_Commands(microsecs)']]},
            "SlowestCalls": {'data': [i[0] for i in data['Slowest_Calls']], 'slowest_calls': [i[1] for i in data['Slowest_Calls']]}
        }
